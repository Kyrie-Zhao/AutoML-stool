...
class Solver_Train(object):
    ...
    def load_data_train_coarse_helper(self, df, bristol):
        index_bristol = df.loc[:, 'condition']==bristol
        ...
        ds = ds.map(self.read_image).map(self.augment).batch(self.train_batch_size // 3)
        return ds

    def load_data_train_coarse(self):
        directory = self.data_root
        df = pd.read_csv(os.path.join(directory, 'train_a_b_annotation.csv'))
        max_size = max(df.loc[:, 'condition'].value_counts())
        ds_0 = self.load_data_train_coarse_helper(df, 0)
        ds_1 = self.load_data_train_coarse_helper(df, 1)
        ds_2 = self.load_data_train_coarse_helper(df, 2)
        return ds_0, ds_1, ds_2

    def load_data_train_fine_helper(self, df, bristol):
    ...
    def load_data_train_fine(self):
        directory = self.data_root
        df = pd.read_csv(os.path.join(directory, 'train_a_b_annotation.csv'))
        max_size = max(df.loc[:, 'bristol_type'].value_counts())
        ds_0 = self.load_data_train_fine_helper(df, 0)
        ds_1 = self.load_data_train_fine_helper(df, 1)
        ds_2 = self.load_data_train_fine_helper(df, 2)
        ds_3 = self.load_data_train_fine_helper(df, 3)
        ds_4 = self.load_data_train_fine_helper(df, 4)
        ds_5 = self.load_data_train_fine_helper(df, 5)
        ds_6 = self.load_data_train_fine_helper(df, 6)
...
#         loss_exit0 = class_balanced_cross_entropy_loss(logits_exit0, self.label_placeholder, self.samples_per_cls_coarse)
        opt_exit = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.9, beta2=0.999, epsilon=1e-8)
#         opt_exit = tf.train.MomentumOptimizer(learning_rate=self.lr, momentum=self.momentum)
        train_op = opt_exit.minimize(loss_exit0)
...
#         sess = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))
        ...
        ds_train_0, ds_train_1, ds_train_2 = self.load_data_train_coarse()
        ...
        best_test_acc = 0
        for epoch in range(1, self.epochs_coarse + 1): # self.epochs = 10 * (MAX_SIZE/2)
            step_list = []
            train_error_list = []
            total_loss_list = []
            branch_loss_list = []
            ...
            X_train = tf.concat((X_train_0, X_train_1, X_train_2), 0)
            y_c_train = tf.concat((y_c_train_0, y_c_train_1, y_c_train_2), 0)
            ...
            progress_bar(epoch, self.epochs_coarse + 1)
    ...
    def train_fine(self, action):
        ...
        B_VGG_instance = B_VGGNet(position = action)
        #print("opt fine")
        ...
        update_ops_fine_1 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tmp_scope_fine_1)
        ...
        update_ops_fine_2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tmp_scope_fine_2)
        ...
        update_ops_fine_3 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=tmp_scope_fine_3)

        opt_exit = tf.train.AdamOptimizer(learning_rate=self.lr, beta1=0.9, beta2=0.999, epsilon=1e-8)
        train_op_1 = opt_exit.minimize(loss_exit1, var_list=update_ops_fine_1)
        train_op_2 = opt_exit.minimize(loss_exit2, var_list=update_ops_fine_2)
        train_op_3 = opt_exit.minimize(loss_exit3, var_list=update_ops_fine_3)
        ...
        with tf.Session() as sess:
            ...
            saver = tf.train.Saver()
            print('Start Training Fine'.center(50, '-'))
            ds_train_0, ds_train_1, ds_train_2, ds_train_3, ds_train_4, ds_train_5, ds_train_6 = self.load_data_train_fine()
            batch_0 = ds_train_0.make_one_shot_iterator().get_next()
            batch_1 = ds_train_1.make_one_shot_iterator().get_next()
            batch_2 = ds_train_2.make_one_shot_iterator().get_next()
            ...
            total_loss_list = []
            for epoch in range(1, self.epochs_fine + 1):
                ...
                train_loss, train_error2 = sess.run([train_op_3, train_acc3],
                                                       feed_dict={img_placeholder: X_train_f2,
                                                                  ...
                                                                  training_flag: True})
                total_loss_list.append(train_loss)
                progress_bar(epoch, self.epochs_fine + 1)
            print('End Training Fine'.center(50, '-'))
            #tf.get_variable_scope().reuse_variables()
            save_path = saver.save(sess, os.path.join(self.checkpoint_path, 'AutoML_stool.ckpt'))
            sess.close()

    def test_coarse(self,action):
        tf.reset_default_graph()
        print('Start Testing Coarse'.center(50, '-'))
        coarse_crrect = 0
        coarse_num = 0
        ...
        label_list = []
        pred_list = []
        for i in range(self.epochs_test):
            if i%100==0:
    ...
    def test(self, action):
        tf.reset_default_graph()
        print('Start Testing Fine'.center(50, '-'))
        coarse_num = 0
        coarse_crrect = 0
        fine1_crrect = 0
        fine2_crrect = 0
    ...
    def flops_Cal(self,fine1_num, fine2_num, fine3_num, action):
        ...
        #Action
        convertPosition=["0/conv1", "1/conv2", "2/max_pool1", "3/conv3", "4/conv4", "5/max_pool2",
                         "6/conv5", "7/conv6", "8/conv7", "9/max_pool3", "10/conv8", "11/conv9",
                         "12/conv10", "13/max_pool4", "14/conv11", "15/conv12", "16/conv13"]
        action_coarse = action[0]
        action_fine_1 = action[1]
        action_fine_2 = action[2]
        action_fine_3 = action[3]
        ...
        [logits_exit0, logits_exit1, logits_exit2, logits_exit3] = B_VGGNet_instance.model(img_placeholder, 
                                                                                           is_train=training_flag)
        ...
        print("coarse_flops: {}, fine1_flops: {}, fine2_flops: {}, fine3_flops: {}, total flops: {} MFLOPS".format(
            coarse_flops,fine1_flops,fine2_flops,fine3_flops,flops))
        sess.close()
        return flops
...
def main():
    ...
    solver = Solver_Train(position=[16, 16, 16, 16])
    solver.train()
